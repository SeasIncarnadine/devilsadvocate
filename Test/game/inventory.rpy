...
screen inventory_screen():    
    ...
    for item in evidence:
        if i+1 <= (evidence_page+1)*10 and i+1>evidence_page*10:
            ...
            imagebutton idle pic hover pic xpos x ypos y action [ Play ("sound", "sfx/click.wav"), description_textbox.Action(item.description), SetVariable('selectedEvidence', item)]# unhovered [description_textbox.Action("")]# hovered [ Play ("sound", "sfx/click.wav"), description_textbox.Action(item.description)] unhovered [description_textbox.Action("")]# at highlight_evidence
        $ i += 1
        if len(evidence)>10:
