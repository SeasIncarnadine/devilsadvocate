...
screen inventory_screen():
    ...
    if crossExaminationInProgress:
        textbutton "Present" action If(selectedEvidence is not None, [Stop("music"), Play("shout", "sfx/Phoenix - takethat.mp3", loop=False), Function(endCrossExamination), Hide("inventory_screen"), Show("inventory_button"), Show("profiles_button"), Jump(currentCrossExaminationTag + "_" + str(currentCrossExaminationStatement) + "_present")]) align (.5,.04)
...
screen profiles_screen():
    ...
    frame:
        ...
        text description_textbox.value color "#000000"
    if crossExaminationInProgress:
        textbutton "Present" action If(selectedEvidence is not None, [Stop("music"), Play("shout", "sfx/Phoenix - takethat.mp3", loop=False), Function(endCrossExamination), Hide("profiles_screen"), Show("inventory_button"), Show("profiles_button"), Jump(currentCrossExaminationTag + "_" + str(currentCrossExaminationStatement) + "_present")]) align (.5,.04)
